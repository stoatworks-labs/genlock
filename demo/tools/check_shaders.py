...
`demo/plugin.js` holds the two GLSL shaders that `source/Shaders.cpp` holds.
...
Nothing else can. `gltest` drives the real plugin class through the real FFGL
...
reasoning that justifies the code: why each input has its own MaxUV, why
smoothstep's upper edge is held a hair above its lower one.

The one transformation is a decode, not a normalisation. A shader that
quotes a name in a comment with backticks, and a backtick cannot
...
# JS constant, C++ file, C++ symbol. Two, because the plugin has one pass: the
# pass-through vertex shader (UV unscaled, MaxUV applied per fetch) and the
# genlock itself.
SHADERS = [
    ("VERTEX_SHADER", "source/Shaders.cpp", "kVertexShader"),
    ("GENLOCK_SHADER", "source/Shaders.cpp", "kGenlockShader"),
]
